        ...
        */
        ...
        getLinks: function (current_page, eventHandler, turnHandler, changeHandle) {
            ...
            if (this.opts.is_show_first_last && current_page>0) {
                $("<a class='first'>首页</a>").attr('href', this.opts.link_to.replace(/__id__/, 0)).data('page_id', 0).appendTo(fragment);
            }
            ...
            {
                $("<a class='last'>尾页</a>").attr('href', this.opts.link_to.replace(/__id__/, np - 1)).data('page_id', np - 1).appendTo(fragment);
            }
            if (this.opts.items_per.length > 0) {
                var items = [], div = $("<div class='page-selector'>每页<select></select>条</div>"), select = div.find("select");
            ...
            {
                var input = $("<div class='page-input'>到第<input type='text' maxlength='10' class='page-text' />页</span>&nbsp;<input class='btn select-page-btn' type='button' value='跳转' ></div>");
