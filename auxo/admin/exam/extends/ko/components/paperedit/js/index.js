...
(function (w, $) {
    function Model(params) {
        ...
        this.store = {
            ...
            get: function (projectCode, examId) {
            ...
            update: function (projectCode, examId, data) {
                var url = '/' + projectCode + '/v1/exams/' + examId + '/strategy';
                return $.ajax({
                    url: url,
                    type: 'put',
            ...
            online: function (projectCode, examId) {
            ...
            getQuestionBanks: function (projectCode) {
                var url = '/' + projectCode + '/v1/questionbanks?page=0&size=1000&type=2';
                return $.ajax({
                    url: url,
                    type: 'get',
                    dataType: "json",
            ...
            getPapers: function (projectCode, search) {
    ...
    Model.prototype = {
        ...
        _init: function () {
            ...
            this._validator();
            this.store.getQuestionBanks(this.params.projectCode).done(function (data) {
        ...
        searchPapers: function () {
            ...
            this.store.getPapers(this.params.projectCode, _paperFilter).done(function (data) {
        ...
        list: function (flag) {
            var self = this;
            this.store.get(this.params.projectCode, this.params.examId)
                .done(function (data) {
                    if (flag) {
        ...
        doSave: function (callback) {
            ...
            this.store.update(this.params.projectCode, this.params.examId, data).done(function () {
                if (callback) {
                    callback();
                }
            });
        ...
        //返回上一步按钮事件
        toExam: function () {
            this.params.preStepCallBack && this.params.preStepCallBack();
        },
        online: function () {
            var _self = this;
        ...
        //取消按钮事件
        cancel: function () {
            this.params.cancelCallBack && this.params.cancelCallBack();
        },
        //保存按钮事件
        save: function () {
            this.doSave($.proxy(function () {
                ...
                this.params.saveCallBack && this.params.saveCallBack();
            }, this));
        },
        //保存并返回按钮事件
        saveThenReturn: function () {
            this.doSave(this.params.saveThenBackCallBack);
        },
