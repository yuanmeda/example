...
ko.components.register('catalogModal', {
    ...
    template: '<div class="modal fade" data-bind="attr:{id:model.id}"><div class="modal-header">'+
                ...
                '<div class="modal-footer"><button class="btn btn-primary" data-bind="click:model.saveCatalog">保存</button><button class="btn" data-dismiss="modal" aria-hidden="true">关闭</button></div>'+
                '</div>'
});
...
ko.components.register('courseItem', {
    ...
    '<li data-check class="item cf">' + 
      '<img class="item-img" data-bind="attr: { src: logo_url, \'data-id\': id }" alt="">' + 
        '<div class="item-dl">' + 
        ...
        '<div class="item-label-box">' +
            '<i class="item-label" data-bind="text: status == 0 ? \'下线\' : \'上线\',css:{ \'on\':!status,\'\':status }"></i>' +
        '</div>' +
...
ko.components.register('otherItem', {
    ...
    template:
        ...
        '</a>' +
        '<div class="item-label-box">' +
            '<i class="item-label" data-bind="text: status == 0 ? \'下线\' : \'上线\',css:{ \'on\':status == 0,\'\':status == 2 }"></i>' +
        '</div>' +
        '<a class="item-remove" href="javscript:;" data-bind="click:function(){$parent.event({id:id})}">' +
            '<i class="icon-close"></i>' +
        '</a>' +
        '<div class="mask"></div>' +
    '</li>'
