...
(function ($) {
    var store = {
        ...
        getExamList: function (name,pageNum) {
            var url =  name ? apiServiceDomain + "/v1/online_exams/search?$filter="+encodeURIComponent("name like '%"+ name +"%'")+"&$offset="+pageNum*20+"&$result=pager&$limit=20" : apiServiceDomain + "/v1/online_exams/search?$offset="+pageNum*20+"&$result=pager&$limit=20";
            return $.ajax({
                url: url,
                type: 'GET',
                cache: false,
        ...
        getExamById: function (id) {
            var url =  apiServiceDomain + "/v1/online_exams/" + id;
            return $.ajax({
                url: url,
                type: 'GET',
                cache: false,
    ...
    var viewModel = {
        ...
        edit: function (id) {
            window.location.href = 'http://' + location.host + '/' + projectCode + '/admin/online_exam/setting?online_exam_id=' + id;
        },
        create: function () {
            window.location.href = 'http://' + location.host + '/' + projectCode + '/admin/online_exam/create';
        },
        clear: function () {
