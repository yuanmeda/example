; (function ($) {
    ...
    var store = {
        /*操作日志查询*/
        getOperateLog: function (user_id_sr, ip_address, moduleName, op_object, action, log_date_begin, log_date_end, page_size, current_page) {
            var filter = "$filter=custom_id eq " + periodic_exam_id + "";

            // 拼接查询条件
            filter += user_id_sr ? " and user_id eq " + user_id_sr : "";
            filter += ip_address ? " and ip_address eq '" + ip_address + "'"  : "";
            filter += moduleName ? " and module eq '" + moduleName + "'"  : "";
