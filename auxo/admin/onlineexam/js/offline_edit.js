(function ($) {
    ...
    var viewModel = {
        ...
        init: function () {
            ...
            $.when(store.getUploadSession(), store.get()).done(function (uploadSession, offlineExam) {
                var uploadSessionData = uploadSession[0];
                var offlineExamData = offlineExam[0];
                $.extend(_self.uploadSessions, uploadSessionData);
                ko.applyBindings(_self, document.getElementById('online_exam_setting'));

                var rData = offlineExamData.online_exam_vo;
                // _self.model.
                _self.model.exam.name(rData.name);
                _self.model.exam.exam_area(rData.exam_area);
                _self.model.exam.pass_score(rData.pass_score);
                _self.model.exam.attachments(rData.attachments ? rData.attachments : []);
                _self.model.exam.description(rData.description);
                _self.model.exam.cover(rData.cover);
                _self.model.exam.cover_url(rData.cover_url);


                _self.model.exam.start_time(Date.toJSTime3(rData.start_time));
                _self.model.exam.end_time(Date.toJSTime3(rData.end_time));

                // 时间参数复制

                var time_frame_len = rData.period_config.time_frames&&rData.period_config.time_frames.length;
                var i = 0;
                for (; i < time_frame_len; i++) {
                    if(rData.period_config.type == 3){
                        rData.period_config.time_frames[i].start_time = Date.toJSTime3(rData.period_config.time_frames[i].start_time)
                        rData.period_config.time_frames[i].end_time = Date.toJSTime3(rData.period_config.time_frames[i].end_time)
                    } else {
                        rData.period_config.time_frames[i].start_time = Date.toJSTime2(rData.period_config.time_frames[i].start_time)
                        rData.period_config.time_frames[i].end_time = Date.toJSTime2(rData.period_config.time_frames[i].end_time)
                    }
                }

                _self.model.exam.period_config.type(rData.period_config.type);
                _self.model.exam.period_config.month(rData.period_config.month ? rData.period_config.month : []);
                _self.model.exam.period_config.time_frames(rData.period_config.time_frames ? rData.period_config.time_frames : []);
                _self.model.exam.period_config.week(rData.period_config.week ? rData.period_config.week : []);

                if (rData.period_config.type == 1) {
                    _self.model.week_time_frames(rData.period_config.time_frames)
                } else if (rData.period_config.type == 2) {
                    _self.model.month_time_frames(rData.period_config.time_frames)
                } else if (rData.period_config.type == 3) {
                    _self.model.time_frames(rData.period_config.time_frames)
                }


                if (rData.affiliated_org_node != undefined || rData.affiliated_org_node != null) {
                    _self.model.exam.affiliated_org_node(rData.affiliated_org_node);
                    _self.model.exam.affiliated_org_root(rData.affiliated_org_root);
                    _self.model.belong_config(1);
                }

                _self.model.exam.visible_config.type(rData.visible_config.type);
                _self.model.exam.visible_config.nodes(rData.visible_config.nodes);
        ...
        prepareData: function (callBack) {
            ...
            if (online_exam_id) {
                ...
                store.create(postData).done(function (resData) {
                    ...
                    online_exam_id = resData.id;
                    callBack && callBack(resData.id);
                });
            }
        },
