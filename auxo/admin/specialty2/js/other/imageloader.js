...
(function ($, window) {
    function ImageModel(params) {
        this.original = ' ';
        if (params.original) {
            this.original = ko.unwrap(params.original) || ' ';
        }

        this.hasRule = true;
        if (params.noRuleKeyWord) {
            var noRuleKeyWord = ko.unwrap(params.noRuleKeyWord);
            this.hasRule = !(new RegExp(noRuleKeyWord).test(this.original))
        }
