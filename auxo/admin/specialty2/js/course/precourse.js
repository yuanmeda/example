;(function ($, window) {
    ...
    var store = {
        getSelectedCourse: function () {
            return $.ajax({
                url: '/' + projectCode + '/specialty/courses/' + courseId + '/pre_courses',
                type: 'GET',
                dataType: 'JSON',
                cache: false
            });
        ...
        getUnselectedCourse: function (data) {
            return $.ajax({
                url: '/' + projectCode + '/specialty/courses/' + courseId + '/optional_pre_courses',
                type: 'GET'
            });
        },
    ...
    var viewModel = {
        ...
        getPreCourseList: function (data) {
            this.model.selectedTrainNow([]);
            $.each(data, function (i, v) {
                v.business_course_vo.pre_sort_number = i + 1;
            });
            ko.mapping.fromJS(data, {}, this.model.selectedTrain);
        },
        ...
        formatData: function (d) {
            return {
                ...
                pre_course_list: d.pre_course_list.map(function (item) {
                    return {
                        progress_percent: item.progress_percent,
                        business_course_vo: {
                            id: item.pre_course_id,
                            name: item.pre_course_name
                        }
                    };
                })
            };
        ...
        getCourseConfig: function () {
            ...
            store.getSelectedCourse().then(function (data) {
                data = _self.formatData(data);
                if (data.unlock_type == 1 || data.unlock_type == 2)
                    _self.getPreCourseList(data.pre_course_list);
        ...
        savePreTrain: function () {
            ...
            var sendFn = function () {
                ...
                if (preTrainForUpdate.length == 0)
                    _self.model.selectedTrain([]);


                var data = {
                    "unlock_type": unlockCriteria.unlock_type,
                    "pre_course_list": preTrainForUpdate
                };

                store.updatePreCourse(data).done(function () {
                    if (_self.model.flags.isPretrain() == "0")
                        _self.model.unlock_type("2");
        ...
        searchTrain: function () {
            ...
            store.getUnselectedCourse({exclude_course_ids: exclude_course_ids}).then(function (data) {
