...
(function ($, window) {
    ...
    Label.prototype = {
        ...
        _uploadParamsInit: function () {
            ...
            if (this.storeUrl) {
                var params = [
                    {
                        target: $('#app_logo'),
                        defaultUrl: currentCatalog.app_store_object_url(),
                        storeUrl: _self.storeUrl,
                        cb: function (data) {
                            currentCatalog.app_store_object_id(data[0].id);
                            currentCatalog.app_store_object_url(data[0].relative_url);
                        }
                    }
                ];
                require.config({
                ...
                _self._selfLoadUploadPlugin(params);
            } else {
                _self.store.getParams().done(function (data) {
                    _self.cloud_token = encodeURIComponent(data.cloud_token);
                    _self.storeUrl = data.cloud_url + '/v5/stores/objects/uploads?bucketName=$image&cloud_token=' + data.cloud_token;
                    var params = [
                        {
                            target: $('#app_logo'),
                            defaultUrl: currentCatalog.app_store_object_url(),
        ...
        _selfLoadUploadPlugin: function (params) {
            //加载ICO图片上传模块
            require(["require", "exports", "admin/other/tag/js/upload/upload-2.0"], function (require, exports, upload) {
                $.each(params, function (index, item) {
                    var uploader = item.target.upload({
                        uploadOptions: {
                            uploadUrl: item.storeUrl,
...
//            }
        ...
        _getNode: function (event, treeId, treeNode) {
            ...
            if (treeNode.id !== this.model.currentCatalog.id()) {
                $(".span7.edit-table").fadeOut();
            } else {
                $(".span7.edit-table").fadeIn();
            }
        },
        /**
         * 隐藏工具条
...
//                                redata.app_store_object_id = redata.app_store_object_id;
//                                redata.app_store_object_url = data.app_store_object_url;
    ...
    Label._ajaxHandler = function (url, data, type) {
        return $.ajax({
            ...
            beforeSend: function (xhr) {
                ...
                //xhr.setRequestHeader('Authorization', Nova.getMacToken(type, url, window.location.host));
