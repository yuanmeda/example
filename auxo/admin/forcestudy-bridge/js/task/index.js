;(function ($, viewModel) {
    ...
    $.extend(true, viewModel, {
         ...
         */
        showImportUser: function (item) {
            this.model.importUser.task_id(item.id);
        ...
        importUser: function () {
            ...
            $("body").cover();
            $.ajaxFileUpload({
                url : '/' + projectCode + '/forcestudys2/tasks/' + this.model.importUser.task_id() + '/import',
                secureuri: false,
                ...
                success: function (response, status){
                    ...
                    $.alert({
                        title: "导入成功",
                        content: "成功导入" + (response || "0").match(/\d+/)[0] + "条数据",
                        confirmButton: "确定"
                    });
                    that.list();
                    $("body").uncover();
