...
//        var compatibilityUtils = {
        ...
        placeholder: function (dom) {
            if (!Utils.check.placeholder()) {
                dom = dom || $('body');
                var inputs = dom.find('[placeholder]');
                inputs.each(function (i, item) {
                    item = $(item);
                    var str = item.attr('placeholder');
                    if (str && !item.attr('readonly') && item.is(':visible')) {
                        var style = '';
                        ...
                        if (parseInt(parent.height(), 10) == 0) {
                            ...
                            if (item[0].nodeName.toLowerCase() === 'textarea') {
                                style = 'style="position:absolute;top:0px;left:2px;"';
                            } else {
