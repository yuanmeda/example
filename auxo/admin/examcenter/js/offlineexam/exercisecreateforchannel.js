            ...
            if (tmplId) {
                $.when(store.get(), store.getUploadingInfo()).done($.proxy(function (examInfos, uploadInfos) {
                    ...
                    if (examInfo.beginTime) {
                        this.model.exam.beginTime(Date.format(new Date(Date.formatTimezone(examInfo.beginTime)), 'yyyy-MM-dd HH:mm'));
                    }
                    if (examInfo.endTime) {
                        this.model.exam.endTime(Date.format(new Date(Date.formatTimezone(examInfo.endTime)), 'yyyy-MM-dd HH:mm'));
                    }
        ...
        saveThenBack: function () {
            ...
            this.doSave(function () {
                if (return_url) {
                    ...
                    window.location.href = return_url + (hasParams ? '&' : '?') + 'id=' + tmplId;
                }
            });
        },
        cancel: function () {
