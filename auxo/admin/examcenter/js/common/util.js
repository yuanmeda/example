...
(function ($, w) {
    ...
    w.Utils = {
        ...
        _init: function () {
            ...
            //ajax失败回调
            $(document).ajaxError(function (e, jqXHR, request, errType) {
                if (jqXHR.readyState == 0 || jqXHR.status == 0) {
                    Utils.alertTip('网络出错，请稍后再试', {icon: 2});
                } else {
                    var txt = JSON.parse(jqXHR.responseText);
                    if (txt.cause) {
                        Utils.alertTip(txt.cause.detail || txt.cause.message, {icon: 2});
                    } else {
                        Utils.alertTip(txt.message, {icon: 2});
                    }
                }
            });
            //ajax开始回调
