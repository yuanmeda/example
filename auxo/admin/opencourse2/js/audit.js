...
(function ($, window) {
    ...
    var store = {
        ...
        updateCourseStatus: function (data, status) {
            var url = '/v1/open_courses/status/' + status;
        ...
        updateCourseTop: function (data, top) {
            var url = '/v1/open_courses/top/' + top;
            return $.ajax({
                url: url,
                type: 'PUT',
                data: JSON.stringify(data),
        ...
        getCourseTags: function (courseId) {
            var url = '/v1/open_courses/' + courseId + '/tags';
            return $.ajax({
                url: url,
                cache: false
            });
        ...
        sortCourse: function (targetId, dropId, top, cb) {
            var url = '/v1/open_courses/' + targetId + '/sort';
            return $.ajax({
                url: url,
                cache: false,
                type: "PUT",
        ...
        query: {
            courseInfo: function (courseId) {
                var url = '/v1/courses/' + courseId + '/base';
                return $.ajax({
                    url: url
                });
            }
        ...
        post: {
            /** 审核通过 */
            pass: function (courseId) {
                var url = '/v1/open_courses/' + courseId + '/actions/pass';
            ...
            /** 审核驳回 */
            reject: function (courseId, data) {
                var url = '/v1/open_courses/' + courseId + '/actions/refuse';
            ...
            /** 提交审核 */
            submit: function (courseId, data) {
                var url = '/v1/open_courses/' + courseId + '/actions/submit';
    ...
    var viewModel = {
        ...
        _initOrgTree: function (managerData, treeData) {
            ...
            function uri(treeId, treeNode) {
                return '/v1/open_courses/manage_orgs/' + treeNode.node_id + '/nodes'
            }
        ...
        _init: function () {
            ...
            this.model.audit.wordcount = ko.computed(function () {
                if (this.model.audit.content()) {
                    return 200 - this.model.audit.content().length;
                } else {
                    return 200;
                }
            }, this);
            ...
            store.getMixOrgTree().done($.proxy(function (returnData) {
                this._initOrgTree(returnData.manager, _self.transform(returnData.org_tree));
            }, this));
        },
        /**
         * dom操作事件定义
        ...
        createCourse: function () {
            window.parent.location.href = '/' + projectCode + '/admin/open_course/create?course_id=0'
        },
        ...
        //通用分类事件
        orderSearch: function (audit_status) {
            viewModel.model.filter.audit_status(audit_status);
            this.doSearch();
        },
        ...
        updateAuditAction: function (status) {
            this.model.audit.action(status);
            this.model.audit.content("");
        },
        resolve: function () {
            store.post.pass(this.model.currentCourse.id()).done($.proxy(function (data) {
                this.doSearch();
                $("#cloudcoursedetail").modal("hide");
            }, this));
        },
        reject: function () {
            store.post.reject(this.model.currentCourse.id()).done($.proxy(function (data) {
                this.doSearch();
                $("#cloudcoursedetail").modal("hide");
            }, this));
        },
        audit: function (data) {
            if (data && data.tags && data.tags.length) {
                ...
                if (len === 1) {
                    content = data.tags[0].title;
                    this.model.currentCourse.labelContent(content);
                } else {
            ...
            $("#cloudcoursedetail").modal("show");
            ko.mapping.fromJS(data, {}, this.model.currentCourse);
            this.updateAuditAction(-1);
        }
    };
