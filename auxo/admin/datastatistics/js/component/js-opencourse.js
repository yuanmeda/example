(function(window, $) {
    ...
    var modalTpl = '<div class="modal fade" id="js_opencourse_modal" tabindex="-1" role="dialog" aria-labelledby="js_opencourse_modal" aria-hidden="true">\
                      <div class="modal-dialog modal-lg" style="width:1045px;height:700px;">\
                        <div class="modal-content">\
                          <div class="modal-header">\
                            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>\
                            <h4 class="modal-title" id="js_opencourse_modal">选择公开课</h4>\
                          </div>\
                          <div class="modal-body" style="padding:0">\
                            <iframe id="opencourse_iframe_js" width="1045px" height="700" style="border:0;"></iframe>\
                          </div>\
    ...
    function ViewModel(params) {
        var _uriStr='/'+projectCode+'/open_course?options=true&cors=true';
    ...
    ViewModel.prototype = {
        ...
        _getItems: function() {
            ...
            this.$$opencourse[0].contentWindow.postMessage(JSON.stringify(msg), '*');
        },
        _openModal: function() {
            var vm = this;
            if (!this._initial()) {
                ...
                $(document.body).on('click', '.modal_confirm_js', function() {
                    ...
                    vm.$$opencourse[0].contentWindow.postMessage(JSON.stringify(msg), '*');
                });
                this.$$modal = $('#js_opencourse_modal');
                this.$$opencourse = $('#opencourse_iframe_js');
                this._initial(true);
                this.$$opencourse.attr('src', this.uri + '&__mac=' + this.__mac);
            } else {
                this.$$opencourse.attr('src', this.uri);
            }
            this.$$modal.modal('show');
        }
    };
    ko.components.register('x-statistics-opencourse', {
        viewModel: {
        ...
        template: '<div class="form-group ml10">\
                        <label sr-only>选择公开课</label>\
                        <input type="text" class="form-control" placeholder="选择公开课 >" readonly data-bind="click:_openModal,value:itemsName"/>\
                    </div>'
    })
})(window, jQuery);
