(function(window, $) {
    ...
    var store = {
        ...
        countByStatus: function(status) {
            var uri = '/v1/datastatistics/trains/count';
            return $.ajax({
                url: uri,
                data: {
                    status: status
        ...
        // 趋势概览
        trend: function(day) {
            var uri = '/v1/datastatistics/trains/user_daily_stats';
            return $.ajax({
                url: uri,
                data: {
                    day: day || 7
        ...
        // 参与度排行
        joins: function(top) {
            var uri = '/v1/datastatistics/trains/user_count_rank';
            return $.ajax({
                url: uri,
                data: {
                    top: top || 10
        ...
        // 合格度排行
        passed: function(top) {
            var uri = '/v1/datastatistics/trains/pass_user_count_rank';
            return $.ajax({
                url: uri,
                data: {
                    top: top || 10
    ...
    ViewModel.prototype = {
        ...
        _countByStatus: function() {
            var vm = this;
            $.when(store.countByStatus(0), store.countByStatus(1))
                .then(function(d1, d2) {
                    vm.model.count.online(d2[0].count);
                    vm.model.count.offline(d1[0].count);
                    vm.model.count.total(d1[0].count + d2[0].count);
                });
        },
        /**
         * 趋势概览 
        ...
        _chartTrendInit: function(id) {
            ...
            datas.forEach(function(d) {
                ...
                chartData.joins.push(d.user_count);
                chartData.passed.push(d.pass_user_count);
            });
            ...
            store.joins()
                .done(function(d) {
                    ...
                    d.forEach(function(item){
                        chartData.statics.push(item.title);
                        chartData.items.push(item.user_count);
                    });
                    vm._chartModuleInit('part_chart_js','培训参与度排行',chartData,'{c}人');
                });
        },
        /**
         * 合格度
        ...
        _passed: function() {
            ...
            store.passed()
                .done(function(d) {
                    ...
                    vm._chartModuleInit('pass_chart_js','培训合格率排行',chartData,'{c}%');
                });
        },
        /**
         * moduleechart表格初始化
