...
(function (window, $) {
    ...
    var store = {
        ...
        getCourseHours: function () {
            var url = window.elearning_business_course_url + '/v1/business_courses/' + courseId + '/time_rules';
            return $.ajax({
                url: url,
                cache: false
            });
        ...
        getExtraInfos: function () {
            var url = window.elearning_business_course_url + '/v1/business_courses/' + courseId + '/extra_infos';
            return $.ajax({
                url: url,
                cache: false
            });
        ...
        getResourceTime: function (data) {
            var url = window.elearning_business_course_url + '/v1/business_courses/' + courseId + '/resource_time';
            return $.ajax({
                url: url,
                type: 'POST',
                data: JSON.stringify(data),
        ...
        //获取前端信息展示方式
        getPeriod: function () {
            var url = window.elearning_business_course_url + '/v1/business_courses/' + courseId + '/config';
            return $.ajax({
                url: url,
                cache: false
            });
    ...
    var viewModel = {
        ...
        init: function () {
            ...
            this.model.adjustedConfig.subscribe(function (val) {
                if (!val) this.model.courseHourRule.adjusted_time(0);
            }, this);
            ...
            //监听
            store.getCouseBaseInfo().then(function (data) {
                // if (data.sub_type == 1) {
                //     $('#offline_hoursetting').show();
                // } else if (data.sub_type == 0) {
                //     $('#hoursetting').show();
                // }
                if (data.business_type == 'offline_course') {
                    $('#offline_hoursetting').show();
                } else {
                    $('#hoursetting').show();
                }
        ...
        _toSave: function () {
            ...
            $.when(store.updateCourseHours(_courseHourRule), store.updatePeriod(periodData))
                .done(function () {
        ...
        // 保存下线课程学时
        saveOfflineCourse: function () {
            var self = this;
            var offlineCoueseHourSetting = ko.unwrap(self.model.offlineCoueseHourSetting) + '';

            if(!/^\d+(\.\d+)?$/.test(offlineCoueseHourSetting)) {
                $.fn.dialog2.helpers.alert('课程学时必须为整数或者小数')
                return;
            }
            if(offlineCoueseHourSetting > 277) {
                $.fn.dialog2.helpers.alert('课程学时必须小于277');
                return;
            }
            var adjusted_time = parseInt(offlineCoueseHourSetting * 3600);
            return store.updateCourseHours({
                enable_custom: false,
                adjusted_time: adjusted_time,
                video_effect_max: 0,
                document_effect_max: 0,
                exercise_effect_max: 0,
                vr_effect_max: 0
            }).then(function () {
                $.fn.dialog2.helpers.alert("保存成功", {
                    close: function () {
