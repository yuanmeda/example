(function ($) {
    var store = {
        ...
        getPeriodicExam: function () {
            var url = apiServiceDomain + '/v1/periodic_exams/' + periodicExamId;
            return $.ajax({
                url: url,
                type: 'GET',
                cache: false,
    ...
    var viewModel = {
        ...
        _getPeriodicExam: function () {
            ...
            if (periodicExamId) {
                store.getPeriodicExam().done(function (resData) {
                    _self.model.periodicExam(resData);
        ...
        saveQuestionBank: function (bankIds, questionBanks, bankFlag) {
            var params = viewModel.model.periodicExam();
            ...
            if (periodicExamId) {
                store.updatePeriodicExam(params).done(function () {
                    window.location.href = 'http://' + location.host + '/' + projectCode + '/admin/periodic_exam/paperstrategy?periodic_exam_id=' + periodicExamId + '&return_url=' + encodeURIComponent(returnUrl);
                });
            }
        },
        save: function () {
        ...
        cancel: function () {
            ...
            window.location.href = 'http://' + location.host + '/' + projectCode + '/admin/periodic_exam/setting?periodic_exam_id=' + periodicExamId;
        }
    };
    $(function () {
        viewModel.init();
