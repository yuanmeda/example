(function ($) {
    ...
    var viewModel = {
        ...
        gopage: function (item) {
            ...
            window.open('/' + projectCode + '/admin/periodic_exam/admin/session_users?periodic_exam_id=' + item.periodic_exam_id);
        },

        goMarkSetting: function (item) {
            var item = ko.mapping.toJS(item);
            window.open(manualMarkGatewayDomain + '/' + projectCode + '/admin/manual_mark/' + item.exam_id + '/manual_mark_setting?__mac=' + Nova.getMacToB64(manualMarkGatewayDomain + '/' + projectCode + '/admin/manual_mark/' + item.exam_id + '/manual_mark_setting') + '#/correct-setting');
        }
    };
    $(function () {
        viewModel.init();
