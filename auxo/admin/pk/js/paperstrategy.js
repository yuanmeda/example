    ...
    var viewModel = {
            ...
            if (pk_id) {
                store.get().done($.proxy(function (data) {
                    if (data.exam_paper_strategy && data.exam_paper_strategy.paper_strategy && !data.exam_paper_strategy.paper_strategy.paper_part_strategies) {
                        data.exam_paper_strategy.paper_strategy.paper_part_strategies = [];
                    }
                    if (data.exam_paper_strategy && data.exam_paper_strategy.paper_strategy && data.exam_paper_strategy.paper_strategy.question_bank_ids) {
                        this.model.pkExam.exam_paper_strategy.paper_strategy.question_bank_ids(data.exam_paper_strategy.paper_strategy.question_bank_ids);
                    }
                    ko.mapping.fromJS(data, {}, _self.model.pkExam);
                    ko.applyBindings(this, document.getElementById('periodicexam-paper-strategy'));
                }, this));
            } else {
                //todo
            }
        ...
        saveThenReturnEvent: function () {
            ...
            if (pk_id) {
                store.updatePkExam(params).done(function () {
                    if (return_url) {
                        var match = return_url&&return_url.match(/[?&]id=([^=?&])/)
                        if(match){
