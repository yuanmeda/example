(function ($) {
    var store = {
        ...
        exam: {
            actionRules: function() {
              var url = window.self_host + "/v1/periodic_exam/" + window.periodic_exam_id + "/action_rules?action=periodicexam_web_access&_=" + new Date().getTime();
              return $.ajax({
                  url: url,
                  type: 'GET',
                  dataType: 'JSON',
            ...
            session: function (examId, data) {
                var url =  window.self_host + "/v1/user_exam_sessions?exam_id=" + examId;
                return $.ajax({
                    url: url,
                    type: 'POST',
                    data: JSON.stringify(data),
    ...
    var viewModel = {
        ...
        init: function () {
            ...
            $.when(store.exam.info(), store.exam.actionRules()).done($.proxy(function(examInfo, actionRules) {
                this.model.periodicExamDetail = examInfo[0];
                this.model.actionRules = actionRules[0];
                this.initExamInfo();
                var prepareConfig = this.getPrepareConfig();
                this.play(prepareConfig);
            }, this))

            // store.exam.info().done($.proxy(function (data) {
            //   this.model.periodicExamDetail = data;
        ...
        getPrepareConfig: function () {
            var config = {
                ...
                userExamStatus: this.model.actionRules.result ? this.model.periodicExamDetail.periodicExamUserStatus : -1,
                ...
                enrollExam: function () {
                    var that = this;
                    location.href = TokenUtils.getEnrollUrl(data.examId, location.href);
                },
        ...
        getExamTimeText: function () {
            if (this.model.periodConfig && (this.model.periodConfig.type != undefined)) {
