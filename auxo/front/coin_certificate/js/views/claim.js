...
(function ($, window) {
    ...
    var store = {
        ...
        getCoinInfo: function () {
            return $.ajax({
                url: window.self_host + '/v1/coin_certificates/' + coinId,
                dataType: 'json',
                cache: false
            });
        },
        ...
        updateCoinInfo: function (data) {
            return $.ajax({
                url: window.self_host + '/v1/coin_certificates/' + coinId + '/use_situations',
                type: 'post',
        ...
        getUserInfo: function (mobile) {
            return $.ajax({
                url: window.self_host + '/v1/uc/users/actions/third_query',
                data: mobile,
                dataType: 'json',
                cache: false
            });
    ...
    var viewModel = {
         ...
         */
        _init: function () {
            document.title = _getKeyValue('coin.mobile.pageTitle');
            //ko监听数据
        ...
        _getRegCoupon: function () {
            ...
            if (userId) {
                store.updateCoinInfo({
                    ...
                    _self._popBox(_getKeyValue('coin.mobile.congratulation'));
                })
            }
        },
            ...
            if (patt.test(mobileNum) && +mobileNum) {
                store.getUserInfo(getData).done(function (data) {
                    if (data && data.user_id) {
                        ...
                        _self._popBox(_getKeyValue('coin.mobile.regClosed'));
                    } else {
                        window.location.href = elearningWebfrontUrl + '/' + projectCode + '/account/m/register?returnurl=' + encodeURIComponent(window.location.href.replace(/&?user_id=.+/g, '')) + '&mobile=' + _self.model.mobileNum();
                    }
                })
            } else {
                _self._popBox(_getKeyValue('coin.mobile.inputNumber'));
            }
        },
        /**
