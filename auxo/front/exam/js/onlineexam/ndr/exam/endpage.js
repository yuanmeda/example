(function ($) {
    ...
    require(['studying.exam.answer', 'studying.end'], function (v, p) {
        ...
        $.when($.ajax({
            ...
            var urlOrNot = false;
            // mineData.beginTime = Date.ajustTimeString(mineData.beginTime); //mineData.beginTime.insert(mineData.beginTime.indexOf("+") + 2, ":");
            // mineData.endTime = Date.ajustTimeString(mineData.endTime); //mineData.endTime ? mineData.endTime.insert(mineData.endTime.indexOf("+") + 2, ":") : null;
            // if (mineData.userData && mineData.userData.finishTime)
            //     mineData.userData.finishTime = Date.ajustTimeString(mineData.userData.finishTime); //mineData.userData.finishTime.insert(mineData.userData.finishTime.indexOf("+") + 2, ":");
            // if (mineData.userData && mineData.userData.markTime)
            //     mineData.userData.markTime = Date.ajustTimeString(mineData.userData.markTime); //mineData.userData.markTime.insert(mineData.userData.markTime.indexOf("+") + 2, ":");
            // if (mineData.userData && mineData.userData.startTime)
            //     mineData.userData.startTime = Date.ajustTimeString(mineData.userData.startTime); //mineData.userData.startTime.insert(mineData.userData.startTime.indexOf("+") + 2, ":");
            // if (mineData.userData && mineData.userData.submitTime)
            //     mineData.userData.submitTime = Date.ajustTimeString(mineData.userData.submitTime); //mineData.userData.submitTime.insert(mineData.userData.submitTime.indexOf("+") + 2, ":");
            ...
            mineData.beginTime = mineData.beginTime ? $.format.toBrowserTimeZone(Date.ajustTimeString(mineData.beginTime), 'yyyy/MM/dd HH:mm:ss') : null;
            mineData.endTime = mineData.endTime ? $.format.toBrowserTimeZone(Date.ajustTimeString(mineData.endTime), 'yyyy/MM/dd HH:mm:ss') : null;
            if (mineData.userData && mineData.userData.finishTime)
            ...
            $('#end').end({
                ...
                title: mineData.name,
                analysisUrl: "http://" + window.location.host + selfUrl + '/' + projectCode + "/exam/analysis?location_source=" + locationSource,
                restartUrl: "http://" + window.location.host + selfUrl + '/' + projectCode + "/exam/prepare/" + examId + "?ranking_url=" + (rankingUrl ? rankingUrl : '') + '&location_source=' + locationSource + "&custom_data=" + (customData ? encodeURIComponent(customData) : ""),
