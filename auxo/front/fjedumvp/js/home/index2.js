(function ($, window) {
    var store = {
        ...
        setAdminRole: function (roleName) {
            return $.ajax({
                url: '/fjedu/admin/role/set/' + userId,
                type: 'POST',
                data: JSON.stringify({
                    role_name: roleName
                })
        ...
        getHaveClass: function (userId) {
            var url = userCenterUrl + '/v2.0/users/' + userId + '/have_classes';
            return $.ajax({
                url: url
            });
        },
    ...
    var viewModel = {
        ...
        clickRole: function (obj) {
            this.model.adminRole(obj);
        },
        roleSelected: function () {
            var t = this;
            var roleName = t.model.adminRole();
            if (!roleName) {
                t.popClose();
                return;
            }

            store.setAdminRole(roleName).done(function () {
                location.href = '/home';
            });
        },
        popClose: function () {
        ...
        haveClass: function () {
            if (userId) {
                store.getRoleName(userId).done(function (d) {
                    if (d.is_visitor == 0) {//非体验账号
                        store.getHaveClass(userId).done(function (d) {
                            if (!d)
                                $('#js_popTip').show();
                        });
                    }
