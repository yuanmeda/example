(function ($) {
    ...
    function getMacKey() {
        ...
        if(!config) return "";
        var gmac = base64_decode(decodeURIComponent(getCookie(config.cookie_mac_key)));
        var mackey = JSON.parse(gmac).mac_key;
        return mackey
    }
    ...
    require(["studying.exercise.factory"], function (factory) {
        ...
        var config = {
            "host": {
                "mainHost": (typeof examUrl == 'undefined' ? "http://elearning-exam-api.debug.web.nd" : examUrl), // 必填，测评组件API地址
                "resourceGatewayHost": resourceGatewayUrl,                                          // 必填，资源网关API地址
                "answerGatewayHost": answerUrl,
                "noteServiceHost": null,                                                            // 必填，笔记组件API地址
                "questionBankServiceHost": null,                                                     // 必填，题库服务API地址
                "errorUrl": null,                                               // 必填，初始化过程中发生流程错误是用户显示错误信息的页面，值为完整的URL地址
                "returnUrl": selfUrl + '/' + projectCode + '/wrong_question/result?sessionId=' + sessionId + '&type=' + type // 必填，结果页面地址
            },
            "envConfig": {
                "examId": "",
                "sessionId": sessionId,                                                               // 必填，当前会话标识
                "macKey": getMacKey(),                                                                     // 必填，mac_key
                ...
                "tokenConfig": {
                    ...
                    "onGetToken": function (context) {
                        return {
                            ...
                            "X-Gaea-Authorization": JSON.parse(base64_decode(decodeURIComponent(G_CONFIG))).encode_gaea_id
                        };
            ...
            "eventCallBack": {
                ...
                "onSubmitCallBack": function (userPaperAnswerVo) {
                    if (userPaperAnswerVo.latestAnswerTime) {
                        var parts = userPaperAnswerVo.latestAnswerTime.match(/\d+/g);
                        var date = new Date(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5]);
                        $.cookie('user_latest_answer_time', date.getTime(), {expires: 7, path: '/'});
                    } else {
                        $.cookie('user_latest_answer_time', null);
                    }
                }
            }
        }
        ...
        bussiness.init();
        window.parent.bussiness = bussiness;
    });
}(jQuery));