    ...
    var dialogHelper = function (msg) {
        if ($.fn.udialog) {
            $.fn.udialog.alert(msg, {
                icon: 'error',
                title: window.i18nHelper ? window.i18nHelper.getKeyValue('platform_dev.dialog.title') : '系统提示',
                buttons: [{
                    text: window.i18nHelper ? window.i18nHelper.getKeyValue('platform_dev.dialog.confirm') : '确认',
                    click: function () {
                        $(this).udialog("hide");
