;(function (window) {
    ...
    // 角色选择引导弹窗显隐
    edupf.popGuide = function () {
        var _popGuide = $('.pop-guide'),
            _box = _popGuide.find('.box'),
            _switchLanguage = $('.switch-language'),
            nameWidth = $('.login-name').width(),
            newBoxWidth = 70 + nameWidth,
            newImgRight = 54 + parseInt(newBoxWidth / 2);

        // 根据有无中英文按钮来设置弹窗内容位置
        if (_switchLanguage.length) {
            _popGuide.addClass('pop-guide-lang');
        }

        _box.css('width', newBoxWidth + 'px');
        _popGuide.find('.icon-teacher').css('right', newImgRight + 'px');

        // 确认按钮
        _popGuide.find('.btn-confirm').on('click', function () {
            _popGuide.fadeOut();
