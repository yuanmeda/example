    ...
    var viewModel = {
        ...
        nextStep: function (res) {
            ...
            switch (this.account.role_name) {
                case 'student':
                    ...
                    $('#js_login_current_role').text(i18nHelper.getKeyValue('header.student'));
                    $('#js_current_role').html(i18nHelper.getKeyValue('header.student') + '<i class="triangle-down"></i>');
                    break;
                case 'teacher':
                    $('.role-content a[data-role="teacher"]').addClass('checked');
                    $('#js_login_current_role').text(i18nHelper.getKeyValue('header.teacher'));
                    $('#js_current_role').html(i18nHelper.getKeyValue('header.teacher') + '<i class="triangle-down"></i>');
                    break;
                case 'guardian':
                    $('.role-content a[data-role="guardian"]').addClass('checked');
                    $('#js_login_current_role').text(i18nHelper.getKeyValue('header.guardian'));
                    $('#js_current_role').html(i18nHelper.getKeyValue('header.guardian') + '<i class="triangle-down"></i>');
                    break;
            }
        },
        activeCss: function () {
        ...
        go: function () {
            ...
            switch (this.model.type) {
                ...
                case '课程':
                    if (this.account.role_name == 'teacher')
                        url = channelUrl + '/tedu/search?keyword=' + keyword;
                    else if (this.account.role_name == 'guardian')
                        url = channelUrl + '/pedu/search?keyword=' + keyword;
                    else
                        url = channelUrl + '/edu/search?keyword=' + keyword;
                    break;
                default:
