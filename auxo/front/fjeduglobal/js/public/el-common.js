            ...
            store.getNotices().done(function (d) {
                ...
                if (length > 0) {
                    for (var i = 0; i < length; i++) {
                        var item = d.items[i];
                        if (item.is_readed == 0) {
                            str.push('<li class="clearfix"><a class="title" href="' + mainUrl + '/ls?nav=12">' + item.title + '</a><a class="status" href="' + mainUrl + '/ls?nav=12">查看</a></li>');
                        }
                    }
                    if (str.length > 0) {
        ...
        navs: function () {
            ...
            store.getNav().done(function (d) {
                ...
                if (location.href.indexOf('/famous') >= 0)
                    inStr = '名校名师';
                if (location.href.indexOf('hide_channel_list') >= 0)
                    inStr = '公共平台';
