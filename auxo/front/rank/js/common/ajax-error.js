$(function () {
    var dialogHelper = function (msg) {
        if ($.fn.udialog) {
            $.fn.udialog.alert(msg, {
                icon: 'error',
                title: i18nHelper.getKeyValue('rank.hint'),
                buttons: [{
                    text: i18nHelper.getKeyValue('rank.confirm'),
                    click: function () {
                        $(this).udialog("hide");
    ...
    $(document).ajaxError(function (e, response, request, errType) {
        ...
        if (error) dialogHelper((error.cause ? error.cause.message : error.message) || i18nHelper.getKeyValue('rank.error'));
    });
});
