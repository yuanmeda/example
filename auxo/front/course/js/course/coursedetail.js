(function ($, window) {
    ...
    var viewModel = {
        model: {
            ...
            envirUrl: (window.examWebFrontUrl || "") + '/' + projectCode + '/exam/prepare/',
            autoRgisterFlag: 1,
            courseInfo: {
        ...
        _init: function () {
            ...
            if (statusCode != '7' && (!window.SWITCHES || (window.SWITCHES && window.SWITCHES['switch.assist.fav']))) {
        ...
        switchContentTab: function (index) {
            ...
            if (index == 7) {
                this.dataForPk = {
                    course: {
                        id: window.courseId,
                        name: window.courseName,
                        catalog: JSON.parse(window.catalogInfoJson)
                    },
                    options: {
                        urls: {
                            api: '',
                            gateway: window.pkGatewayUrl,
                            'static': window.staticUrl,
                        },
         ...
         * @return {null}              null
         */
        goToStudy: function (uuid, resourceType, flag, courseId, resourceId) {
            try {
                // 尝试运行exe文件中的对象,若失败会直接报错
                if (CoursePlayer && typeof (CoursePlayer) !== 'undefined' && CoursePlayer.invokeMethodPPT){
                    var eventData ={
                        "vrResourseUrl":courseId+','+resourceId
                    }
                    eventData = JSON.stringify(eventData);
                    var eventName = 'VRResourse';
                    // 将文件给客户端
                    CoursePlayer.invokeMethodPPT(eventName, eventData);
                }
            } catch(e) {
                // 出现错误后,表示属于网页环境，依然保留原来的跳转方式
                console.log(e);
                var $resourceList = $('.catalog-list .resource-item'),
                _href, _self = this;
                //catalogId = $resourceList.find('div[data-uuid=' + uuid + ']').attr("data-id");
                if (!flag) {
                    _href = $resourceList.eq(0).attr('href');
                } else {
                    _href = (window.selfUrl || '') + '/' + projectCode + '/course/' + courseId + '/learn?resource_uuid=' + uuid + '&resource_type=' + resourceType;
                }
                if (!_href) {
        ...
        disableTip: function ($data, evt) {
            ...
            switch (statusCode) {
                ...
                case '7':
                    ...
                    break;
                case '8':
                    Utils.msgTip(i18nHelper.getKeyValue('courseComponent.front.courseDetail.tip16'), {
                        icon: 2,
                        time: 1500
                    });
                    break;
        ...
        // 点击打开PC端
        getPpt: function(url,courseId,resourceId) {
            var eventData ={
                "vrResourseUrl":courseId+','+resourceId
            }
            eventData = JSON.stringify(eventData);
