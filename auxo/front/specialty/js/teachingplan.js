...
(function () {
    ...
    var service = {
        ...
        // 查询课程类别
        getContainers: function (specialtyPlanId) {
            return $.ajax({
                url: window.envconfig.service + '/v1/teaching_plans/' + specialtyPlanId + '/containers'
            })
        },
        // 获取开课时间
        getCourseTime: function (specialtyPlanId) {
            return $.ajax({
                url: window.envconfig.service + '/v1/specialty_plan_exts/' + specialtyPlanId
            })
        },
        // 获取教学计划表
        getTeachingPlans: function (specialtyPlanId) {
            return $.ajax({
                url: window.envconfig.service + '/v1/teaching_plans/' + specialtyPlanId,
                type: 'GET'
            })
        },
        // 获取教学计划系统生成的表格
        getTeachingPlanTable: function (specialtyPlanId) {
            return $.ajax({
                url: window.envconfig.service + '/v1/teaching_plans/' + specialtyPlanId + '/tables',
                type: 'GET'
            })
        },
        // 获取教学计划导入的表格
        getTeachingPlanImportTable: function (specialtyPlanId) {
            return $.ajax({
                url: window.envconfig.service + '/v1/teaching_plans/' + specialtyPlanId + '/import_tables',
                type: 'GET'
            })
        },
        /**
        ...
        getCourses: function (specialtyPlanId, data) {
            return $.ajax({
                url: auxoSpecCourseWebpageHost + '/' + PROJECT_CODE + '/teachingplans/' + specialtyPlanId + '/courses',
                cache: false,
                type: 'GET',
                dataType: 'json',
                data: data
    ...
    ViewModel.prototype = {
        ...
        initViewModel: function (element) {
            ...
            this.model.teachingPlanTable = ko.observable({name: '', course_containers: [], counter: {}}),
            // 年级/课程TAB/学期/学分
        ...
        initData: function () {
            ...
            (function () {
                store.defer(function () {
                    ...
                    that.model.valueToGetCourse.subscribe(function (v) {
                        // 只有当前TAB是课程且课程类别不为空时才激活查询课程
                        if (that.model.init.course() && that.model.courseContainer().length) {
                            that.getCourses();
                        }
                    });
                    that.model.courseContainer.subscribe(function (v) {
                        // 只有当前TAB是课程时才激活查询课程
        ...
        getSpecialtyPlan: function () {
            ...
            service.getSpecialtyPlan(data).then(function (data) {
                ...
                // 获取课程类别
                (function () {
                    service.getContainers(data.specialty_plan_id).then(function (res) {
                        that.model.courseContainer(res)
                    })
                }());

                // 获取开课时间
                (function () {
                    service.getCourseTime(data.specialty_plan_id).then(function (res) {
                        var timeStart = new Date(res.time_start).getTime()
                        var timeEnd = new Date(res.time_end).getTime()
                        var currentTime = new Date().getTime()
                        if (res.time_type === 2) {
                            if (currentTime < timeStart) {
                                that.model.courseTimeStatus(0)
                            } else if (currentTime >= timeStart && currentTime <= timeEnd ) {
                                that.model.courseTimeStatus(1)
                            } else if (currentTime > timeEnd) {
                                that.model.courseTimeStatus(2)
                            }
                        }
                        that.model.courseStartTime(moment(res.time_start).format('YYYY-MM-DD HH:mm:ss'))
                    })
                })();

                // 获取教学计划，无计划时不显示教学计划tab
                (function () {
                  service.getTeachingPlans(data.specialty_plan_id).then(function (res) {
                      if (res) {
                          that.model.showTeachingPlansTab(true)
                          that.model.teachingPlanType(res+'')
                          if (res === 1) {
                              // 系统生成
                              service.getTeachingPlanTable(data.specialty_plan_id).then(function(ret){
                                ret.course_containers.forEach(function(element, k) {
                                    var count = 0
                                ...
                                that.model.teachingPlanTable(ret);
                              })
                          } else if (res === 2) {
                              // 我的上传
                              service.getTeachingPlanImportTable(data.specialty_plan_id).then(function(data){
         ...
         */
        getCourseContainerId: function (type) {
            var id = ''
            var containers = this.model.courseContainer()
            for (var i = 0; i < containers.length; i++) {
                if (containers[i].indexOf(type) > -1) {
                    id = containers[i].course_container_id
                    break
                }
            }
            return id
        ...
        getCourses: function () {
            var that = this,
                containers = this.model.courseContainer(),
                courses = [],
                index = 0,
            ...
            for (var i = 0; i < containers.length; i++) {
                (function (i) {
                    data.container_id = containers[i].course_container_id
            ...
            // 取必修课
            // (function () {
            //     data.container_id = that.getCourseContainerId('必修')
            //     service.getCourses(specialtyPlanId, data).then(function (data) {
            ...
            // 取选修课
            // (function () {
            //     data.container_id = that.getCourseContainerId('选修')
            //     service.getCourses(specialtyPlanId, data).then(function (data) {
            ...
            // 取实践课
            // (function () {
            //     data.container_id = that.getCourseContainerId('实践')
            //     service.getCourses(specialtyPlanId, data).then(function (data) {
            //         that.model.practiceCourseList(data.items || [])
