            ...
            ExerciseAnswer.prototype.dofinish = function () {
                ...
                this.store.commit().done(function () {
                    _this.store.end().done(function (data) {
                        if (resultPageUrl) {
                            var defaultUrl = resultPageUrl + _this.store.data.ExamId + '/' + _this.store.data.SessionId;
    ...
    (function (Learning) {
        ...
        (function (Bussiness) {
            var StandardExercise = function (_super) {
                ...
                StandardExercise.prototype.viewAnalysis = function () {
                    var _this = this;
                    $.learning.loading.show();
                    this.main = null;
                    $.when(this.getExamInfo(), this.getSessionInfo()).done(function (examData, mineData) {
                        ...
                        if (data.userData && data.userData.submitTime)
                            data.userData.submitTime = $.format.toBrowserTimeZone(Date.ajustTimeString(data.userData.submitTime), 'yyyy/MM/dd HH:mm:ss');
                        $.learning.loading.hide();
                        _this._viewAnalysis(data, examData, [], []);
                    }).fail($.proxy(this.onError, this));
                };
                StandardExercise.prototype._viewAnalysis = function (mine, exam, answersData, analysisData) {
                    var config = this._getConfig(mine, exam, answersData, analysisData);
                    config.Exercise.Mode = 3;
                    $.learning.loading.show();
                ...
                StandardExercise.prototype._getConfig = function (mine, exam, answersData, analysisData) {
                    var config = {
