...
Model.prototype = {
    ...
    formatDateTime: function (data) {
        var html = '',
            timeArr,
            dataInfo = data.model.data.unit_info.extra,
            extra=data.model.data.extra;
        if (extra&&(this.getExamStatus() == '60' || this.getExamStatus() == '70')) {
            timeArr = dataInfo.end_time ? dataInfo.end_time.split('T') : '';
            html = dataInfo.end_time ? timeArr[0] + ' ' + timeArr[1].substr(0, 5) + '<em class="ml">' + i18nHelper.getKeyValue('collection_exam_unitcard.ending') + '</em>' : '';
        } else if (extra&&this.getExamStatus() == '50') {
            html = '';
        } else {
            timeArr = dataInfo.begin_time ? dataInfo.begin_time.split('T') : '';
            html = dataInfo.begin_time ? timeArr[0] + ' ' + timeArr[1].substr(0, 5) + '<em class="ml">' + i18nHelper.getKeyValue('collection_exam_unitcard.starting') + '</em>' : "";
        }
        return html;
    },
    ...
    onClick: function () {
        ...
        if (this.model.data.unit_info.extra.subtype == "3")
            url = this.model.host + "/exam/tounament/prepare/" + this.model.data.unit_id;
        else
            url = this.model.host + "/exam/prepare/" + this.model.data.unit_id;
        window.open(url);
    },
    disableBtn: function () {
        var isDisable = -1,
            dataInfo = this.model.data.extra;
    ...
    userExamStatus: function (data) {
        var txt = '',
            dataInfo = data.model.data.extra;
        switch (dataInfo.last_status) {
            case 1:
                txt = 'collection_exam_unitcard.aboutToBegin';
                break;
            case 4:
                txt = 'collection_exam_unitcard.start';
                break;
            case 8:
            case 112:
                txt = 'collection_exam_unitcard.continueExam';
                break;
            case 16:
            case 101:
                txt = 'collection_exam_unitcard.waitForCorrect';
                break;
            case 32:
                txt = (dataInfo.remain_chance <= 0) ? 'collection_exam_unitcard.noChance' : 'collection_exam_unitcard.restart';
                break;
            case 64:
            case 80:
            case 96:
                txt = 'collection_exam_unitcard.end';
                break;
            case 0:
                txt = "collection_exam_unitcard.disabled";
                break;
        }
        return txt;
    },
    getExamStatus: function () {
        var dataInfo = this.model.data.extra;
        switch (dataInfo.last_status) {
            case 4:
            case 8:
            case 112:
