...
function ViewModel(params, componentInfo){
    ...
    prm.then(function(res){
         ...
         if(!that.model.readonly()) 
            that.initTree(res.org_tree)
    });
...
ViewModel.prototype.generateNodePath = function(item){
    ...
    switch(len){
        ...
        default:
            ...
            result.path = $.map(mixNodes,function(item){ return item.node_name }).join(' - ');
            break; 
    }
    return result;
}
...
ViewModel.prototype.removeNode = function($data, isNative){
    ...
    if(value && typeof value !== 'number')
       model.selectedNodeIds.remove($data.node_id)
    else
       model.selectedNodeIds(-1);
      
}
ViewModel.prototype.search = function(){
    var text = this.model.text,
        c = this.treeComponent
    var nodeIds = text ? $.map(c.getNodesByParamFuzzy('node_name', text),function(item){ return item.id }) : [];
