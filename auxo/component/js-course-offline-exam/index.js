import tpl from './template.html'
import ko from 'knockout'

function Model(params) {
    this.model = {
        "data": null,
        "host": ""
    }
    this.init(params);
    // $('.js-lazy:not(.loaded)').lazyload({
    //   load: function () {
    //     $(this).addClass('loaded');
    //   }
    // }).trigger('scroll');
}

Model.prototype = {
    init: function (params) {
        this.model.data = params.model;
        this.model.host = params.host;
        this.model.defaultImage = 'data:image/png;base64,'
    },
    formatDateTime: function (data) {
        var html = '',
            timeArr,
            dataInfo = ko.mapping.toJS(data.model.data.unit_info.extra), item = data.model.data, date = '';
        if (item.unit_type == 'online_exam' || item.unit_type == 'offline_exam') {
            if (item.status == 1 || item.status == 2) {
                if (item.unit_info.end_time) {
                    date = item.unit_info.end_time.replace('T', ' ').replace(/-/g, '/').replace(/\.\d+/, '')
                    html = $.format.toBrowserTimeZone(date, 'yyyy/MM/dd HH:mm') + '<em class="ml">' + i18nHelper.getKeyValue('course_offline_unit.ending') + '</em>'
                }
            } else {
                if (item.unit_info.start_time) {
                    date = item.unit_info.start_time.replace('T', ' ').replace(/-/g, '/').replace(/\.\d+/, '');
                    html = $.format.toBrowserTimeZone(date, 'yyyy/MM/dd HH:mm') + '<em class="ml">' + i18nHelper.getKeyValue('course_offline_unit.starting') + '</em>'

                }
            }
        } else {
            if (this.getExamStatus() == '60' || this.getExamStatus() == '70') {
                var date = '';
                if (dataInfo.end_time) {
                    date = dataInfo.end_time.replace('T', ' ').replace(/-/g, '/').replace(/\.\d+/, '')
                    html = $.format.toBrowserTimeZone(date, 'yyyy/MM/dd HH:mm') + '<em class="ml">' + i18nHelper.getKeyValue('course_offline_unit.ending') + '</em>'
                }
            } else if (this.getExamStatus() == '50') {
                html = '';
            } else {
                if (dataInfo.begin_time) {
                    date = dataInfo.begin_time.replace('T', ' ').replace(/-/g, '/').replace(/\.\d+/, '');
                    html = $.format.toBrowserTimeZone(date, 'yyyy/MM/dd HH:mm') + '<em class="ml">' + i18nHelper.getKeyValue('course_offline_unit.starting') + '</em>'

                }
            }
        }
        return html;
    },
    formatScore: function (score) {
        return score ? parseFloat(score).toFixed(1) : 0;
    },
    hrefTo: function () {
        var href = this.getExamPrepareUrl();
        href += (~href.indexOf('?') ? '&' : '?') + '__mac=' + Nova.getMacToB64(href);
        var w = window.open();
        w.location.href = href;
    },
    getExamPrepareUrl: function () {
        if (this.model.data.unit_info.extra.subtype == "3") {
            return this.model.host + "/exam/tounament/prepare/" + ko.unwrap(this.model.data.unit_id);
        } else if (this.model.data.unit_type == 'online_exam') {
            return this.model.host + "/online_exam/detail?online_exam_id=" + ko.unwrap(this.model.data.unit_id);
        } else if (this.model.data.unit_type == 'offline_exam'){
            return this.model.host + "/online_exam/detail?online_exam_id=" + ko.unwrap(this.model.data.unit_id);
        } else {
            return this.model.host + "/exam/prepare/" + ko.unwrap(this.model.data.unit_id);
        }
    },
    disableBtn: function () {
        var isDisable = -1,
            dataInfo = ko.mapping.toJS(this.model.data.extra);
        if (this.model.data.unit_type == 'online_exam' || this.model.data.unit_type == 'offline_exam') {
            switch (this.model.data.status) {
                case 0:
                    isDisable = 1;
                    break;
                case 1:
                    isDisable = 0;
                    break;
                case 2:
                    isDisable = 1;
                    break;
                default:
                    isDisable = 0;
            }
        } else {
            switch (this.getExamStatus()) {
                case 70:
                    isDisable = 1;
                    break;
                case 50:
                case 60:
                    switch (dataInfo.last_status) {
                        case 16:
                        case 101:
                            isDisable = 1;
                            break;
                        case 32:
                            isDisable = (dataInfo.remain_chance <= 0) ? 1 : 0;
                            break;
                        default:
                            isDisable = 0;
                    }
                    break;
            }
        }
        return isDisable;
    },
    userExamStatus: function (data) {
        var txt = '',
            dataInfo = ko.mapping.toJS(data.model.data.extra);
        if (this.model.data.unit_type == 'online_exam' || this.model.data.unit_type == 'offline_exam') {
            switch (this.model.data.status) {
                case 0:
                    txt = 'course_offline_unit.aboutToBegin';
                    break;
                case 1:
                    txt = 'course_offline_unit.start';
                    break;
                case 2:
                    txt = 'course_offline_unit.end';
                    break;
            }
        } else {
            switch (dataInfo.last_status) {
                case 1:
                    txt = 'course_offline_unit.aboutToBegin';
                    break;
                case 4:
                    txt = 'course_offline_unit.start';
                    break;
                case 8:
                case 112:
                    txt = 'course_offline_unit.continueExam';
                    break;
                case 16:
                case 101:
                    txt = 'course_offline_unit.waitForCorrect';
                    break;
                case 32:
                    txt = (dataInfo.remain_chance <= 0) ? 'course_offline_unit.noChance' : 'course_offline_unit.restart';
                    break;
                case 64:
                case 80:
                case 96:
                    txt = 'course_offline_unit.end';
                    break;
                case 0:
                    txt = "course_offline_unit.disabled";
                    break;
            }
        }

        return txt;
    },
    getExamStatus: function () {
        var dataInfo = ko.mapping.toJS(this.model.data.extra);
        switch (dataInfo.last_status) {
            case 4:
            case 8:
            case 112:
                return 50;
            case 16:
            case 101:
            case 32:
                return 60;
            case 0:
            case 64:
            case 80:
            case 96:
                return 70; // 考试已结束
        }
    }
}

ko.components.register('x-offline-examcard', {
    viewModel: Model,
    template: tpl
})
