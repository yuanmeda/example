(function (ko,$) {
'use strict';

ko = 'default' in ko ? ko['default'] : ko;
$ = 'default' in $ ? $['default'] : $;

var img = 'data:image/png;base64,';

function Model(params) {
    var vm = this;
    var opc2_api_host = params.opc2_api_host;
    var course = params.course;
    var enabled = course.enabled;
    var audit_status = window.parseInt(course.life_cycle.audit_status, 10);
    var getI18nKeyValue = window.i18nHelper.getKeyValue;
    var classify = params.options.classify;
    var store = {
        delete_course: function delete_course() {
            return $.ajax({
                url: opc2_api_host + '/v1/opencourses/' + course.unit_id,
                type: 'DELETE',
                cache: false
            });
        }
    };

    vm.title = course.title;
    vm.cover_url = function () {
        var cover_url = course.cover_url;

        return cover_url ? cover_url : img;
    }();
    vm.status = function () {
        var status = void 0;
        if (audit_status === 2 && enabled === true) {
            status = 'audit_pass';
        } else if (audit_status === 2 && enabled === false) {
            status = 'disabled';
        } else if (audit_status === 1) {
            status = 'auditing';
        } else if (audit_status === 3) {
            status = 'audit_reject';
        } else if (audit_status === 0) {
            status = 'un_upload';
        }
        return status;
    }();
    vm.operations = function () {
        switch (vm.status) {
            case 'auditing':
                return [1];
            case 'audit_reject':
                return [1];
            case 'un_upload':
                return [1];
        }
        return [];
    }();
    function getPrice() {
        if (!course.commodity.is_free && course.commodity.price) {
            for (var key in course.commodity.price) {
                return course.commodity.price[key];
            }
        }
        return 0;
    }
    vm.operation_names = [getI18nKeyValue('my_dcu.card.operation.del'), getI18nKeyValue('my_dcu.card.operation.edit'), getI18nKeyValue('my_dcu.card.operation.upload')];
    vm.show_stat_label = classify.name === 'upload';
    vm.extra = course.extra;
    vm.user_count = course.user_count;
    vm.is_free = course.commodity.is_free;
    vm.price = getPrice();
    vm.opinion = course.opinion || '';
    vm.operate = operate;
    vm.del = del;
    vm.hidePopDel = hidePopDel;
    vm.popDel = ko.observable(false);
    vm.linkToCourseDetail = linkToCourseDetail;

    function linkToCourseDetail($data) {
        var detailUrl = window.courseWebpage + '/' + window.projectCode + '/course/' + course.unit_id;
        var editUrl = window.opencourse2GwUrl + '/' + window.projectCode + '/open_course/edit_course?course_id=' + course.unit_id + '&__return_url=' + window.__return_url;
        var nextPageUrl = detailUrl;
        switch ($data.status) {
            case 'audit_pass':
                nextPageUrl = detailUrl;
                break;
            case 'disabled':
                nextPageUrl = detailUrl;
                break;
            case 'auditing':
                nextPageUrl = editUrl;
                break;
            case 'audit_reject':
                nextPageUrl = editUrl;
                break;
            case 'un_upload':
                nextPageUrl = editUrl;
                break;
        }
        window.top.location.href = nextPageUrl;
    }

    function operate(type) {
        switch (type) {
            case 0:
                return popDel();
            case 1:
                return edit();
            case 2:
                return upload();
        }
    }

    function edit() {
        window.top.location.href = window.opencourse2GwUrl + '/' + window.projectCode + '/open_course/edit_course?course_id=' + course.unit_id + '&__return_url=' + window.__return_url;
    }

    function popDel() {
        vm.popDel(true);
    }

    function hidePopDel() {
        vm.popDel(false);
    }

    function del() {
        store.delete_course().pipe(function (res) {
            vm.popDel(false);
        });
    }

    function upload() {
        window.top.location.href = window.opencourse2GwUrl + '/' + window.projectCode + '/open_course/course_upload?__return_url=' + window.__return_url;
    }
}

var tpl = "<div class=\"x-my-dcu-course-card\" data-bind=\"click: $component.linkToCourseDetail.bind($component, $data)\">\r\n  <!--封面-->\r\n  <div class=\"cover\">\r\n    <img width=\"270\" height=\"180\" data-bind=\"attr:{src:cover_url}\">\r\n    <!--ko if:show_stat_label-->\r\n    <span class=\"status-label\"><!--ko translate:{key:'my_dcu.stat_lb.'+status}--><!--/ko--></span>\r\n    <!--/ko-->\r\n  </div>\r\n  <div class=\"course-data\">\r\n    <!--主要课程数据-->\r\n    <div class=\"data-main\">\r\n      <!--标题-->\r\n      <p class=\"title\" data-bind=\"attr:{title:title},text:title\"></p>\r\n      <!--资源-->\r\n      <div class=\"resources\">\r\n        <span><i data-bind=\"text:extra.video_count || 0\"></i><!--ko translate:{key:'my_dcu.card.resource.video'}--><!--/ko--></span> |\r\n        <span><i data-bind=\"text:extra.document_count || 0\"></i><!--ko translate:{key:'my_dcu.card.resource.doc'}--><!--/ko--></span> |\r\n        <span><i data-bind=\"text:extra.exercise_count || 0\"></i><!--ko translate:{key:'my_dcu.card.resource.exercise'}--><!--/ko--></span>\r\n      </div>\r\n    </div>\r\n    <!--脚部附加-->\r\n    <div class=\"data-footer\">\r\n      <!--开通人数（热度）-->\r\n      <span class=\"users\"><i></i><!--ko text:user_count--><!--/ko--></span>\r\n      <!--价格-->\r\n      <!--ko if:!is_free-->\r\n      <span class=\"price pay\"><!--ko text:price--><!--/ko--></span>\r\n      <!--/ko-->\r\n      <!--ko if:is_free-->\r\n      <span class=\"price free\"><!--ko translate:{key:'my_dcu.card.free_label'}--><!--/ko--></span>\r\n      <!--/ko-->\r\n    </div>\r\n  </div>\r\n  <!--操作-->\r\n  <!--ko if:operations.length > 0-->\r\n  <div class=\"operations\">\r\n    <div class=\"toggle\">\r\n      <span class=\"arr\"></span>\r\n      <ul class=\"drop\">\r\n        <!--ko foreach:operations-->\r\n        <li data-bind=\"click:$component.operate.bind($component, $data),clickBubble: false\">\r\n          <i data-bind=\"attr:{'class':'ico-'+$data}\"></i>\r\n          <!--ko text:$component.operation_names[$data]--><!--/ko-->\r\n        </li>\r\n        <!--/ko-->\r\n      </ul>\r\n    </div>\r\n  </div>\r\n  <!--/ko-->\r\n  <!--ko if: opinion-->\r\n  <div class=\"reason-item\">\r\n    <span class=\"highlight\"><!--ko translate:{key:'my_dcu.card.opinion'}--><!--/ko-->：</span>\r\n    <span data-bind=\"text: opinion\"></span>\r\n  </div>\r\n  <!--/ko-->\r\n</div>\r\n<!--ko if: popDel()===true-->\r\n<div class=\"ui-pop\">\r\n  <div class=\"mask\"></div>\r\n  <div class=\"model\">\r\n    <div class=\"model-hd\"><!--ko translate:{key:'my_dcu.card.pop.hint'}--><!--/ko--></div>\r\n    <div class=\"model-bd\">\r\n      <div class=\"tip-warn\"><!--ko translate:{key:'my_dcu.card.pop.delWarning'}--><!--/ko-->？</div>\r\n    </div>\r\n    <div class=\"model-ft\">\r\n      <a href=\"javascript:;\" class=\"ui-btn\" data-bind=\"click:del.bind($component)\"><!--ko translate:{key:'my_dcu.card.pop.confirm'}--><!--/ko--></a>\r\n      <a href=\"javascript:;\" class=\"ui-btn2\" data-bind=\"click:hidePopDel.bind($component)\"><!--ko translate:{key:'my_dcu.card.pop.cancel'}--><!--/ko--></a>\r\n    </div>\r\n    <a href=\"javascript:;\" class=\"btn-close\" data-bind=\"click:hidePopDel.bind($component)\"></a>\r\n  </div>\r\n</div>\r\n<!--/ko-->";

ko.components.register('x-my-dcu-course-card', {
  viewModel: Model,
  template: tpl
});

function Model$1(params) {
  var vm = this;
  var opc2_gw_host = params.opc2_gw_host;
  var opc2_api_host = params.opc2_api_host;
  var page_size = 20;
  var init_status = window.status || 1;
  var get_upload_courses_xhr = void 0;
  var get_upload_courses_stat_xhr = void 0;
  var store = {
    get_upload_courses_stat: function get_upload_courses_stat() {
      return get_upload_courses_stat_xhr = $.ajax({
        url: opc2_gw_host + '/v1/m/open_courses/stat'
      });
    },
    get_income_stat: function get_income_stat() {
      return $.ajax({
        url: opc2_api_host + '/v1/m/open_courses/income/stat'
      });
    },
    get_upload_courses: function get_upload_courses(size, page, status) {
      return get_upload_courses_xhr = $.ajax({
        url: opc2_gw_host + '/v1/m/open_courses',
        data: {
          page: page,
          size: size,
          status: status
        }
      });
    }
  };

  vm.opc2_api_host = params.opc2_api_host;
  vm.classifies = [{
    name: 'audit_pass',
    status: 1
  }, {
    name: 'disabled',
    status: 2
  }, {
    name: 'auditing',
    status: 3
  }, {
    name: 'audit_reject',
    status: 4
  }, {
    name: 'un_upload',
    status: 5
  }, {
    name: 'upload',
    status: 0
  }];
  vm.is_loading = ko.observable(true);
  vm.is_list_loading = ko.observable(false);
  vm.getClassifyByStatus = getClassifyByStatus;
  vm.curr_classify = ko.observable(vm.getClassifyByStatus(init_status));
  vm.list = ko.observableArray([]);
  vm.courses_stat = ko.observable({});
  vm.income_stat = ko.observable({});
  vm.total_pages = ko.observable(-1);
  vm.page_no = ko.observable(0);
  vm.card_options = {};

  vm.change_tab = change_tab;
  vm.load_more = load_more;
  vm.go_upload = go_upload;

  get_courses_stat().pipe(get_income_stat).pipe(function () {
    if (vm.courses_stat().upload_courses_stat === 0) {
      return;
    }
    return get_courses(0, init_status);
  }).always(function () {
    vm.is_loading(false);
  });

  function postMsgToParent() {
    var msg = {
      "type": "$resize",
      "data": {
        "width": document.documentElement.scrollWidth + 'px',
        "height": document.documentElement.scrollHeight + 'px'
      },
      "origin": location.host,
      "timestamp": +new Date()
    };
    window.parent.postMessage(JSON.stringify(msg), '*');
  }

  function getClassifyByStatus(status) {
    for (var i = 0; i < vm.classifies.length; i++) {
      if (status == vm.classifies[i].status) {
        return vm.classifies[i];
      }
    }
  }

  function change_tab(classify) {
    vm.curr_classify(classify);
    vm.list([]);
    get_courses(0, classify.status, true);
  }

  function get_income_stat() {
    return store.get_income_stat().pipe(function (res) {
      vm.income_stat(res);
    });
  }

  function get_courses_stat() {
    return store.get_upload_courses_stat().pipe(function (res) {
      vm.courses_stat(res);
    });
  }

  function get_courses(page, status, is_refresh_stat) {
    get_upload_courses_xhr && get_upload_courses_xhr.abort();
    get_upload_courses_stat_xhr && get_upload_courses_stat_xhr.abort();
    vm.is_list_loading(true);
    var list = void 0;

    return store.get_upload_courses(page_size, page, status).pipe(function (res) {
      if (page > 0) {
        list = vm.list().concat(res.items);
      } else {
        list = res.items;
      }
      vm.page_no(page);
      var total_pages = Math.ceil(res.total / page_size) - 1;
      vm.total_pages(total_pages);
    }).pipe(function () {
      if (!is_refresh_stat) {
        return;
      }
      return get_courses_stat();
    }).pipe(function () {
      vm.card_options.classify = vm.curr_classify();
      vm.list(list);
      setTimeout(function () {
        postMsgToParent();
      }, 200);
    }).always(function () {
      vm.is_list_loading(false);
    });
  }

  function load_more() {
    get_courses(vm.page_no() + 1, vm.curr_classify().status, true);
  }

  function go_upload() {
    window.top.location.href = window.opencourse2GwUrl + '/' + window.projectCode + '/open_course/course_upload?__return_url=' + window.__return_url;
  }
}

var tpl$1 = "\r\n<!--ko if:is_loading()-->\r\n<div class=\"x-my-dcu-loading\" data-bind=\"translate:{key:'my_dcu.loading'}\"></div>\r\n<!--/ko-->\r\n\r\n<!--ko if:!is_loading()-->\r\n<!--无数据-->\r\n<!--ko if:courses_stat().upload_count === 0-->\r\n<div class=\"x-my-dcu-nothing-course\">\r\n  <div class=\"ico\"></div>\r\n  <p class=\"dsc\" data-bind=\"translate:{key:'my_dcu.nothing_course'}\"></p>\r\n  <p><span data-bind=\"click:$component.go_upload.bind($component), translate:{key:'my_dcu.go_upload'}\"></span></p>\r\n</div>\r\n<!--/ko-->\r\n<!--有数据-->\r\n<!--ko if:courses_stat().upload_count > 0-->\r\n<!--统计-->\r\n<div class=\"x-my-dcu-stat\">\r\n  <div class=\"stat-col upload-count\">\r\n    <h1 data-bind=\"translate:{key:'my_dcu.stat.upload_count'}\"></h1>\r\n    <p>\r\n      <span><strong data-bind=\"text:courses_stat().upload_count\"></strong><!--ko translate:{key:'my_dcu.stat.unit_course'}--><!--/ko--></span>\r\n    </p>\r\n  </div>\r\n  <div class=\"stat-col earnings\">\r\n    <h1 data-bind=\"translate:{key:'my_dcu.stat.earnings'}\"></h1>\r\n    <p>\r\n      <span><strong data-bind=\"text:income_stat().CHANNEL_EMONEY || 0\"></strong><!--ko translate:{key:'my_dcu.stat.unit_platform_coin'}--><!--/ko--></span>\r\n      <span><strong data-bind=\"text:income_stat().CHANNEL_CASH || 0\"></strong><!--ko translate:{key:'my_dcu.stat.unit_rmb'}--><!--/ko--></span>\r\n      <span><strong data-bind=\"text:income_stat().CHANNEL_GOLD || 0\"></strong><!--ko translate:{key:'my_dcu.stat.unit_point'}--><!--/ko--></span>\r\n    </p>\r\n  </div>\r\n</div>\r\n<!--tab切换-->\r\n<div class=\"x-my-dcu-tab\">\r\n  <!--ko foreach:classifies-->\r\n  <span data-bind=\"\r\n    css:{on:$component.curr_classify().name===name},\r\n    click:$component.change_tab.bind($component, $data)\r\n  \">\r\n    <!--ko translate:{key:'my_dcu.stat_lb.'+name}--><!--/ko-->\r\n    (<!--ko text:$component.courses_stat()[name+'_count']--><!--/ko-->)\r\n  </span>\r\n  <!--/ko-->\r\n</div>\r\n\r\n<!--列表-->\r\n<!--ko if:list().length > 0-->\r\n<div class=\"x-my-dcu-list\">\r\n  <!--ko foreach:list-->\r\n  <!--ko component:{name:'x-my-dcu-course-card', params:{course:$data, options:$component.card_options, opc2_api_host: $component.opc2_api_host}}--><!--/ko-->\r\n  <!--/ko-->\r\n\r\n  <!--分页-->\r\n  <!--ko if:page_no() < total_pages() && !is_list_loading()-->\r\n  <span class=\"load-more\" data-bind=\"click:load_more, translate:{key:'my_dcu.load_more'}\"></span>\r\n  <!--/ko-->\r\n  <!--ko if:page_no() < total_pages() && is_list_loading()-->\r\n  <span class=\"load-more disable\" data-bind=\"translate:{key:'my_dcu.loading_more'}\"></span>\r\n  <!--/ko-->\r\n</div>\r\n<!--/ko--><!--endif list.length>0-->\r\n\r\n<!--空列表，未在加载中且列表为空时-->\r\n<!--ko if:list().length === 0 && !is_list_loading()-->\r\n<div class=\"x-my-dcu-empty\">\r\n  <div class=\"ico\"></div>\r\n  <p data-bind=\"translate:{key:'my_dcu.list_empty'}\"></p>\r\n</div>\r\n<!--/ko-->\r\n\r\n<!--加载中，在加载中且列表为空时-->\r\n<!--ko if:list().length === 0 && is_list_loading()-->\r\n<div class=\"x-my-dcu-loading\" data-bind=\"translate:{key:'my_dcu.loading'}\"></div>\r\n<!--/ko-->\r\n\r\n<!--/ko--><!--endif courses_stat().upload_count > 0-->\r\n<!--/ko--><!--endif !is_loading-->";

ko.components.register('x-my-dcu', {
  viewModel: Model$1,
  template: tpl$1
});

}(ko,$));
