  ...
  (function () {
    ...
    Fn[X] = Fn[H] = Fn[Q] = Fn[nn] = Fn[tn] = Fn[rn] = Fn[en] = Fn[un] = Fn[on] = true, Fn[z] = Fn[B] = Fn[J] = Fn[D] = Fn[M] = Fn[q] = Fn[K] = Fn["[object Map]"] = Fn[V] = Fn[Z] = Fn[Y] = Fn["[object Set]"] = Fn[G] = Fn["[object WeakMap]"] = false;var Ln = {};Ln[z] = Ln[B] = Ln[J] = Ln[D] = Ln[M] = Ln[X] = Ln[H] = Ln[Q] = Ln[nn] = Ln[tn] = Ln[V] = Ln[Z] = Ln[Y] = Ln[G] = Ln[rn] = Ln[en] = Ln[un] = Ln[on] = true, Ln[q] = Ln[K] = Ln["[object Map]"] = Ln["[object Set]"] = Ln["[object WeakMap]"] = false;var Nn = { "\xc0": "A", "\xc1": "A", "\xc2": "A", "\xc3": "A", "\xc4": "A", "\xc5": "A", "\xe0": "a", "\xe1": "a", "\xe2": "a",
      ...
      "'": "'", "\n": "n", "\r": "r", "\u2028": "u2028", "\u2029": "u2029" },
        ...
        Yn = Mn && qn && _typeof(commonjsGlobal) == "object" && commonjsGlobal && commonjsGlobal.Object && commonjsGlobal || Vn !== (this && this.window) && Vn || Kn || this,
        Gn = function () {
      try {
        Object({ toString: 0 } + "");
      } catch (n) {
...
function ViewModel(params) {
    this.model = {
        currentTime: ko$1.observable(""),
        resourceConfigType: ko$1.observableArray([]),
        subscribes: {
            items: ko$1.observableArray([]),
            init: ko$1.observable(false),
            counts: ko$1.observable(0),
            search: {
                page_size: 20,
                page_no: ko$1.observable(0)
            }
        },
        api: {
...
ViewModel.prototype.store = {
    ...
    getResourceConfigType: function getResourceConfigType() {
        return $.ajax({
    ...
    readSubscribe: function readSubscribe() {
        return $.ajax({
    ...
    getSubscribes: function getSubscribes(params) {
        return $.ajax({
    ...
    unsubscribe: function unsubscribe(subscribe_id) {
        return $.ajax({
...
ViewModel.prototype.getSubscribes = function () {
    ...
    subscribes.items([]);
    this.store.getSubscribes(ko$1.mapping.toJS(subscribes.search)).then(function (res) {
        $.each(res.items, function (i, item) {
            $.each(item.resources.items, function (j, resource) {
                resource.resource_id = resource.unit_id;
            });
            item.resources.showMore = ko$1.observable(false);
        });
    ...
    dialog.showDialog({
        ...
        confirm: function confirm() {
            self.store.unsubscribe(data.subscribe_id).done(function (res) {
                ...
                lodash.remove(subscribes.items(), data);
...
ViewModel.prototype.hrefToResource = function ($data, typeKey) {
    var resourceConfigType = this.model.resourceConfigType;
    var href = formatUrl(lodash.find(resourceConfigType(), { unit_type: $data[typeKey] }).info_url, $data),
...
ViewModel.prototype.hrefToList = function (data) {
    var tag_id = "";
    if (data.tags.length) {
        tag_id = data.tags.map(function (tag) {
            return tag.id;
        }).join(",");
    }
...
ko$1.components.register('x-usercenter-subscribe', {
    viewModel: ViewModel,
    template: tpl
});
