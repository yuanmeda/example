;(function ($, window) {
    ...
    QuestionListModel.prototype.loadPlayer = function () {
        ...
        this.model.questions.qtiItems(_.arrayMap(self.model.questions.items(), function (item) {
            if (item.questionItem.items != undefined) {
                item.qtiplayer = QtiPlayer.createPlayer({
                    'swfHost': self.options.staticUrl + 'auxo/addins/flowplayer/v1.0.0/',
                    'unifyTextEntry': true,
                    'refPath': self.ref_path
                });
            } else if(item.questionItem.res_type === '$RE0459' || item.questionItem.res_type === '$RE0460' || item.questionItem.res_type === '$RE0461' || item.questionItem.res_type === '$RE0462' || item.questionItem.res_type === '$RE0463'){
                var webfrontHost = auxoExamWebfront.replace('http://', 'https://');
                item.tabLink = webfrontHost + '/' + projectCode + '/exam/detection/reading/' + window.examId + '?question_id=' + item.questionItem.id;
            } else {
                item.tabLink = examFrontDomain+'/' + projectCode + '/exam/icplayer/index?_lang_=zh_CN&inject=answerFlow&question_id=' + item.questionItem.id;
            }
            return item;
        }));
