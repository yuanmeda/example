...
Model.prototype = {
    ...
    formatDateTime: function (data) {
        ...
        if (item.unit_type == 'online_exam') {
            if (item.status == 1 || item.status == 2) {
                if (item.unit_info.end_time) {
                    date = item.unit_info.end_time.replace('T', ' ').replace(/-/g, '/').replace(/\.\d+/, '')
                    html = $.format.toBrowserTimeZone(date, 'yyyy/MM/dd HH:mm') + '<em class="ml">' + i18nHelper.getKeyValue('exam_unitcard.ending') + '</em>'
                }
            } else {
                if (item.unit_info.start_time) {
                    date = item.unit_info.start_time.replace('T', ' ').replace(/-/g, '/').replace(/\.\d+/, '');
                    html = $.format.toBrowserTimeZone(date, 'yyyy/MM/dd HH:mm') + '<em class="ml">' + i18nHelper.getKeyValue('exam_unitcard.starting') + '</em>'
    ...
    disableBtn: function () {
        ...
        if (this.model.data.unit_type == 'online_exam') {
    ...
    userExamStatus: function (data) {
        ...
        if (this.model.data.unit_type == 'online_exam') {
            switch (this.model.data.status) {
                case 0:
                    txt = 'exam_unitcard.aboutToBegin';
                    break;
                case 1:
                    txt = 'exam_unitcard.start';
                    break;
                case 2:
                    txt = 'exam_unitcard.end';
                    break;
            }
        } else {
            switch (dataInfo.last_status) {
                ...
                case 8:
                case 112:
                    txt = 'exam_unitcard.continueExam';
                    break;
                case 16:
                case 101:
                    txt = 'exam_unitcard.waitForCorrect';
                    break;
                case 32:
                    txt = (dataInfo.remain_chance <= 0) ? 'exam_unitcard.noChance' : 'exam_unitcard.restart';
                    break;
                case 64:
                case 80:
                case 96:
...
ko.components.register('x-examcard', {
    viewModel: Model,
    template: tpl
})
