;(function () {
    ...
    var CARDSIZEMAP = {4: {h: 314, w: 270, mb: 40}, 5: {h: 256, w: 218, mb: 28}, 6: {h: 206, w: 180, mb: 24}};

    function ViewModel(params) {
        this.model = {
            type: params.type || 2, // 2 橱窗推荐  6 推荐课程
            id: params.window.id || 0,
            modal: {
                tag_name: ko.observable('window_tag' + params.window.id),
                tag: ko.observable(false),
                tab: ko.observable(false),
                tab_name: ko.observable('window_tab' + params.window.id),
                rule_name: ko.observable('window_rule' + params.window.id)
            },
            items: ko.observableArray([]),
            setting: {
                data: {
                    title: ko.observable(params.setting.data.title || ""), //橱窗标题 ,
                    data_type: ko.observable(params.setting.data.data_type || 1), //数据来源类型 1标签 2资源
                    order_type: ko.observable(params.setting.data.order_type || 1), //排序方式 1最新 2最热 3推荐 data_type为1时有效
                    group_names: ko.observableArray(params.setting.data.group_names || []), //学习单元分组列表 data_type为1时有效
                    tag_id: ko.observable(params.setting.data.tag_id || ""), //标签id（为空，表示显示频道或者资源池下的所有资源）
                    tag_name: ko.observable(params.setting.data.tag_name || ''),
                    channel_id: ko.observable(params.setting.data.channel_id || ''), //标签所属的频道id（为空，表示资源池下的标签）
                    resources: ko.observableArray(params.setting.data.resources || []), //"resource_id": "", //资源标识"channel_id": "" //资源来源频道标识
                    rules: ko.observableArray(params.setting.data.rules || [])
                },
                display: {
                    is_title_show: ko.observable(params.setting.display.is_title_show),
                    web: { //web端展示方式
                        row_num: ko.observable(params.setting.display.web.row_num || 2), //行数 display_type为2时有效
                        column_num: ko.observable(params.setting.display.web.column_num || 4), //列数 display_type为2时有效
                        show_num: ko.observable(params.setting.display.web.show_num || 3), //翻页 type为6时有效
                        ad_enable: ko.observable(params.setting.display.web.ad_enable || false),
                        ad_logo: ko.observable(params.setting.display.web.ad_logo || ""),
                        ad_url: ko.observable(params.setting.display.web.ad_url || "")
                    },
                    mobile: { //移动端展示方式
                        display_type: ko.observable(params.setting.display.mobile.display_type || 1), //展示方式 1左右滑动 2田字格
                        display_num: ko.observable(params.setting.display.mobile.display_num || 5), //显示资源数 display_type为1时有效
                        row_num: ko.observable(params.setting.display.mobile.row_num || 2), //行数 display_type为2时有效
                        column_num: ko.observable(params.setting.display.mobile.column_num || 2), //列数 display_type为2时有效
                        show_num: ko.observable(params.setting.display.mobile.show_num || 2), //翻页 type为6时有效
                        ad_enable: ko.observable(params.setting.display.mobile.ad_enable || false),
                        ad_logo: ko.observable(params.setting.display.mobile.ad_logo || ""),
                        ad_url: ko.observable(params.setting.display.mobile.ad_url || "")
                    }
                }
            },
            ...
            window_data_list: params.window.window_data_list,
            rule_data_list: ko.observableArray(params.window.rule_data_list || []),
        ...
        this.model.sizeHint = ko.computed(function () {
            ...
            var size = CARDSIZEMAP[display.web.column_num()];
    ...
    ViewModel.prototype.init = function () {
        ...
        if (model.type == 2) {
            ...
            this.autoSave(setting.display.web.show_num);
            this.autoSave(setting.display.mobile.show_num);
        }
        ...
        this.autoSave(setting.display.web.row_num);
        this.autoSave(setting.display.web.column_num);
        this.autoSave(setting.display.mobile.display_type);
        this.autoSave(setting.display.mobile.display_num);
        this.autoSave(setting.display.mobile.row_num);
        this.autoSave(setting.display.mobile.column_num);
        this.autoSave(setting.display.is_title_show);
    };
    // flag:1 返回文字; flag：0 返回bool
    ViewModel.prototype.formatStatus = function ($data, flag) {
        if ($data.resource_id) {
    ...
    ViewModel.prototype.create = function (items) {
        ...
        this.store.create({setting: JSON.stringify(setting)}, this.model.id)
            .done(function () {
                ...
                that.model.parent.getChangeState.call(that.model.parent);
                that.drag();
            });
    };
            ...
            buttons: {
                confirm: {
                    ...
                    action: function () {
                        ...
                        if (that.model.type == 6) {
                            that.model.rule_data_list.remove(function (item) {
                                return item.id == $data.id;
                            });
                        }
                        that.model.parent.model.isChange('true');
                        that.create(ko.mapping.toJS(that.model.items));
                    }
                },
                cancel: {
                    text: '取消',
    ...
    ViewModel.prototype.openRuleModal = function (id, index, status) {
    ...
    ViewModel.prototype.updateRule = function ($data) {
        var t = this, count = 0;
    ...
    ViewModel.prototype.saveAd = function () {
    ...
    ko.components.register('x-channel-window', {
        synchronous: true,
        viewModel: ViewModel,
