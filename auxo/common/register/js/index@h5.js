;(function (window, $, ko) {
    var searchArr = location.search.replace(/&/g,'=').split('='), mobile= '', returnUrl = '';
    $.each(searchArr, function (i, v) {
        if (~v.toLowerCase().indexOf('mobile')) {
            mobile = decodeURIComponent(searchArr[i + 1]);
        }
    ...
    var store = {
        ...
        register: function (data) {
            return $.ajax({
                url:  "/" + projectCode + "/accounts/register",
                dataType: "json",
                type: "POST",
    ...
    var viewModel = {
        ...
        code: function () {
            ...
            var mobile = $.trim(this.model.user.mobile()),
                that = this;
            var data = {mobile: mobile, sign: getMD5Value(mobile + mobile.split('').reverse().join('')), country_code: this.model.country.code()};
            store.code(data)
                .done(function () {
                    that.countZero();
                })
        ...
        reg: function () {
            ...
            user.mobile_code = $.trim(user.mobile_code);
                ...
                .done(function (res) {
                    if (returnUrl) {
                        if(~returnUrl.indexOf('?')){
                            location.href = returnUrl + '&user_id=' + res.user_id;
                        }else{
                            location.href = returnUrl + '?user_id=' + res.user_id;
                        }

                    } else {
                        location.href = elearning_web_url + '/enrolls/user/login';
                    }
                    fn(1);
                })
                .fail(function (res) {
