        ...
        */
        createLink: function (page_id, current_page, appendopts) {
            ...
            if (page_id == current_page) {
                // lnk = isNaN(parseInt(appendopts.text, 10)) ? $("<span class='current' data-bind=\"translate: {" + 'key:' + '\'' + appendopts.text + '\'' + "}\"></span>") : $("<span class='current'>" + appendopts.text + "</span>");
                ...
                // lnk = $("<a data-bind='translate : { key: \'" + appendopts.text + "\'}'></a>").attr('href', this.opts.link_to.replace(/__id__/, page_id));
            }
        ...
        getInfo: function (current_page) {
            ...
            return $("<div class='pagination-info pull-left'><span>" + i18nHelper.getKeyValue('common.addins.pagination.overview', { 'currentPage': currentPage, 'totalPage': totalPage, 'totalCount': totalCount }) + "</span></div>");
        ...
        getMiniInfo: function (current_page) {
            ...
            return $("<div class='pagination-info pull-left'><span>" + i18nHelper.getKeyValue('common.addins.pagination.minOverview', { 'currentPage': currentPage, 'totalPage': totalPage, 'totalCount': totalCount }) + "</span></div>");

            // return $("<div class='pagination-info pull-left'><span>第 " + currentPage + " / " + totalPage + " 页，共 " + totalCount + " 条</span></div>");
            // return $("<div class='pagination-info pull-left'><span data-bind=\"translate: { key: \'common.addins.pagination.minOverview\', properties: {" + "currentPage:" + currentPage + ", totalPage:" + totalPage + ", totalCount:" + totalCount + " } }\"></span></div>");
        },
        getLinks: function (current_page, eventHandler, turnHandler, changeHandle) {
            ...
            if (this.opts.is_show_first_last && current_page > 0) {
                ...
                // $("<a class='first' data-bind=\"translate: { key: \'common.addins.pagination.first\' }\"></a>").attr('href', this.opts.link_to.replace(/__id__/, 0)).data('page_id', 0).appendTo(fragment);
            }
            ...
            if (this.opts.is_show_first_last && (current_page != np - 1)) {
                ...
                // $("<a class='last' data-bind=\"translate: { key: \'common.addins.pagination.last\' }\"></a>").attr('href', this.opts.link_to.replace(/__id__/, np - 1)).data('page_id', np - 1).appendTo(fragment);
            }
            if (this.opts.items_per.length > 0) {
                var items = [], div = $("<div class='page-selector'><span>" + i18nHelper.getKeyValue('common.addins.pagination.eachPage') + "</span><select></select>" + i18nHelper.getKeyValue('common.addins.pagination.record') + "</div>"), select = div.find("select");
